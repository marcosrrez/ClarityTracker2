...
export const supervisorInsightSchema = z.object({
  ...
  isRead: z.string().default('false').transform(val => val === 'true'),
  category: z.string().optional(),
  actionRequired: z.string().default('false').transform(val => val === 'true'),
...
export const privacySettingsTable = pgTable('privacy_settings', {
  ...
  // Automatic Anonymization Fields
  automaticAnonymization: boolean('automatic_anonymization').default(true),
  piiDetectionLevel: varchar('pii_detection_level', { length: 20 }).notNull().default('standard'),
  preserveTherapeuticContext: boolean('preserve_therapeutic_context').default(true),
  anonymizationReviewRequired: boolean('anonymization_review_required').default(false),
  customAnonymizationRules: jsonb('custom_anonymization_rules').default([]),
...
export const privacySettingsSchema = z.object({
  ...
  automaticAnonymization: z.boolean().default(true),
  piiDetectionLevel: z.enum(['basic', 'standard', 'comprehensive']).default('standard'),
  preserveTherapeuticContext: z.boolean().default(true),
  anonymizationReviewRequired: z.boolean().default(false),
  customAnonymizationRules: z.array(z.string()).default([]),
